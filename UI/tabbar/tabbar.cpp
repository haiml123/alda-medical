...
TabBar::TabBar() : active_tab_index_(0), hovered_tab_index_(-1)
{
}

TabBar::TabBar(const TabBarStyle& style) : active_tab_index_(0), style_(style), hovered_tab_index_(-1)
{
}
    ...
    for (size_t i = 0; i < tabs_.size(); ++i)
    {
        ...
        if (ImGui::IsItemClicked(ImGuiMouseButton_Left))
        {
            if (tabs_[i].enabled && !is_active)
            {
                ...
                tab_changed = true;
                if (on_tab_click_)
                    on_tab_click_(static_cast<int>(i), tabs_[i]);
            }
        }
...
void TabBar::render_tab(int index, const Tab& tab, bool is_active)
{
    ...
    if (style_.browser_style)
    {
        ...
        draw_list->PathArcTo(ImVec2(top_left.x + rounding, top_left.y + rounding), rounding, M_PI, M_PI * 1.5f, 8);
        draw_list->PathLineTo(ImVec2(top_right.x - rounding, top_right.y));
        draw_list->PathArcTo(
            ImVec2(top_right.x - rounding, top_right.y + rounding), rounding, M_PI * 1.5f, M_PI * 2.0f, 8);
        ...
        {
            draw_list->AddLine(
                bottom_left, ImVec2(top_left.x, top_left.y + rounding), border_color_u32, style_.border_thickness);

            draw_list->PathClear();
            draw_list->PathArcTo(ImVec2(top_left.x + rounding, top_left.y + rounding), rounding, M_PI, M_PI * 1.5f, 8);
            draw_list->PathStroke(border_color_u32, false, style_.border_thickness);

            draw_list->AddLine(ImVec2(top_left.x + rounding, top_left.y),
                               ImVec2(top_right.x - rounding, top_right.y),
                               border_color_u32,
                               style_.border_thickness);
            ...
            draw_list->AddLine(
                ImVec2(top_right.x, top_right.y + rounding), bottom_right, border_color_u32, style_.border_thickness);

            draw_list->AddLine(bottom_left, bottom_right, border_color_u32, style_.border_thickness);
        }
    }
    else
    {
        ...
        if (style_.show_borders)
        {
            draw_list->AddRect(rect_min, rect_max, border_color_u32, style_.rounding, 0, style_.border_thickness);
        }
    }
    ...
    ImVec2 text_pos = ImVec2(cursor_pos.x + (actual_size.x - text_size.x) * 0.5f,
                             cursor_pos.y + (actual_size.y - text_size.y) * 0.5f);

    ImU32 text_color =
        is_disabled ? ImGui::GetColorU32(ImVec4(0.5f, 0.5f, 0.5f, 1.0f)) : ImGui::GetColorU32(ImGuiCol_Text);
...
void TabBar::render_add_button()
{
    ...
    else
    {
        ...
        if (style_.show_borders)
        {
            draw_list->AddRect(top_left, bottom_right, border_color_u32, rounding, 0, style_.border_thickness);
        }
    }
    ...
    draw_list->AddLine(
        ImVec2(center.x - plus_size, center.y), ImVec2(center.x + plus_size, center.y), plus_color, thickness);

    draw_list->AddLine(
        ImVec2(center.x, center.y - plus_size), ImVec2(center.x, center.y + plus_size), plus_color, thickness);
...
void TabBar::apply_tab_style(bool is_active, bool /*is_hovered*/, bool is_disabled)
{
    ...
    ImGui::PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2(style_.button_padding_x, style_.button_padding_y));
