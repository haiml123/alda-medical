...
FieldBase::FieldBase(std::string id, std::string label)
    : id_(std::move(id))
...
TextField::TextField(const std::string& id, const std::string& label)
    : FieldBase(id, label)
    , buffer_(256, '\0')
{}
...
void TextField::sync_buffer() {
    if (buffer_.size() < 256) buffer_.resize(256, '\0');
...
bool TextField::validate(const DynamicForm& form) {
    if (!FieldBase::validate(form)) return false;
...
PasswordField::PasswordField(const std::string& id, const std::string& label)
    : FieldBase(id, label)
{
...
IntField::IntField(const std::string& id, const std::string& label)
    : FieldBase(id, label)
{}
...
IntField& IntField::default_value(int val) {
    value_ = val;
    return *this;
}

IntField& IntField::custom_validate(std::function<bool(const DynamicForm&, int)> fn,
                                     const std::string& msg) {
    custom_validator_ = std::move(fn);
    custom_error_ = msg;
    return *this;
...
void IntField::render(float label_width) {
    render_label(label_width);
...
FloatField::FloatField(const std::string& id, const std::string& label)
    : FieldBase(id, label)
{}
...
SelectField::SelectField(const std::string& id, const std::string& label)
    : FieldBase(id, label)
{}
...
void MultilineField::sync_buffer() {
    if (buffer_.size() < 1024) buffer_.resize(1024, '\0');
    if (std::strncmp(buffer_.data(), value_.c_str(), buffer_.size()) != 0) {
        std::strncpy(buffer_.data(), value_.c_str(), buffer_.size() - 1);
    }
}
...
void MultilineField::render(float label_width) {
    ...
    sync_buffer();
    if (ImGui::InputTextMultiline(("##" + id_).c_str(), buffer_.data(), buffer_.size(),
...
CheckboxField::CheckboxField(const std::string& id, const std::string& label)
    : FieldBase(id, label)
{}
...
SectionField::SectionField(const std::string& id, const std::string& label)
    : FieldBase(id, label)
{}
...
CardSelectField::CardSelectField(const std::string& id, const std::string& label)
    : FieldBase(id, label)
{}
...
void CardSelectField::render(float label_width) {
    ...
    for (int i = 0; i < static_cast<int>(options_.size()); i++) {
        ...
        ImVec4 bg_color = is_selected
            ? ImVec4(0.20f, 0.65f, 0.70f, 0.3f)
            : ImVec4(0.12f, 0.13f, 0.15f, 1.0f);
        ImVec4 border_color = is_selected
            ? ImVec4(0.30f, 0.90f, 0.95f, 1.0f)
            : ImVec4(1, 1, 1, 0.1f);
        ImVec4 hover_color = is_selected
            ? bg_color
            : ImVec4(0.15f, 0.16f, 0.18f, 1.0f);
        ...
        if (ImGui::Button(("##card_" + id_ + "_" + std::to_string(i)).c_str(),
                          ImVec2(card_width_, card_height_))) {
            selected_index_ = i;
            dirty_ = true;
        }
        ...
        ImU32 label_color = is_selected
            ? IM_COL32(77, 230, 235, 255)
            : IM_COL32(217, 220, 226, 255);
        ImU32 desc_color = IM_COL32(166, 170, 179, 255);

        draw_list->AddText(
            ImVec2(cursor_pos.x + 12, cursor_pos.y + 16),
            label_color,
            opt.label.c_str()
        );

        draw_list->AddText(
            ImVec2(cursor_pos.x + 12, cursor_pos.y + 40),
            desc_color,
