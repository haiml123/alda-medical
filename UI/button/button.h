...
public:
    ...
    void setStyle(const ButtonStyle& style) { style_ = style; }
    void setSize(const ImVec2& size) { style_.size = size; }
    void setEnabled(bool enabled) { enabled_ = enabled; }
    void setCallback(std::function<void()> callback) { callback_ = callback; }

    // Optional icon support (render icon before label)
    void setIcon(const char* iconText) { icon_ = iconText; }
    void clearIcon() { icon_.clear(); }
    ...
    static bool RenderButton(const std::string& label,
                            const ButtonStyle& style = ButtonPresets::Primary(),
                            bool enabled = true);
