...
struct Channel final : BaseModel
{
    ...
    explicit Channel(const std::string_view name, const std::string_view color = "#FFFFFF")
        : BaseModel(), name(name), color(color)
    {
    }

    // Create channel with id
    Channel(const std::string& id, const std::string_view name, const std::string_view color = "#FFFFFF")
        : BaseModel(id), name(name), color(color)
    {
    }
    ...
    {
        if (filtered != enable || high_pass_cutoff != high_pass || low_pass_cutoff != low_pass)
        {
            filtered = enable;
