...
inline float render_cap_placement_header(const CapPlacementHeaderData& data,
                                         const CapPlacementHeaderCallbacks& callbacks)
{
    ...
    ImGui::BeginChild("##CapPlacementHeader", ImVec2(0, header_h), false, ImGuiWindowFlags_NoScrollbar);
    ...
    char step_text[32];
    snprintf(step_text, sizeof(step_text), "%zu / %zu", data.current_step + 1, data.total_steps);
