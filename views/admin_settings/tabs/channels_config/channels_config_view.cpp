...
void ChannelsConfigView::render(const std::vector<ChannelConfig>& channels,
...
void ChannelsConfigView::render_table(const std::vector<ChannelConfig>& channels,
    ...
    constexpr float table_width = col_ch + col_name + col_type + col_source +
                                  col_diff + col_hpf + col_lpf + col_adf + col_enabled + 20.0f; // +20 for borders
    ...
    if (ImGui::BeginTable("##ChannelsTable", 9, flags, ImVec2(table_width, available_height))) {
        ...
        ImGui::TableSetupColumn("Source",     ImGuiTableColumnFlags_WidthFixed | ImGuiTableColumnFlags_NoResize, col_source);
        ...
        ImGui::TableSetupColumn("Enabled",    ImGuiTableColumnFlags_WidthFixed | ImGuiTableColumnFlags_NoResize, col_enabled);
...
void ChannelsConfigView::render_channel_row(const ChannelConfig& channel,
                                             const ChannelsConfigCallbacks& callbacks) {
    ImGui::TableNextRow();
    ...
    ImGui::TableNextColumn();
    ImGui::PushItemWidth(-1);
    const char* type_items[] = { "EEG", "ECG/EMG", "ICG/Imp", "SCG/MOV", "FPG/Resp",
                                  "GSR", "Temp", "ROT", "BP", "Force" };
