...
AdminSettingsView::AdminSettingsView() {
    ...
    channels_presenter_ = std::make_unique<channels_config::ChannelsConfigPresenter>(
        channels_model_, channels_view_);
}
...
void AdminSettingsView::setup_form() {
    ...
    form_.add_select("nvx_rate", "NVX Rate")
         .options({
             {"10 kHz", 0},
             {"50 kHz", 1},
             {"100 kHz", 2}
    ...
    form_.add_select("nvx_gain", "NVX Gain")
         .options({{"0", 0}, {"1", 1}})
         .width(80.0f);
    ...
    form_.add_select("nvx_scan_freq", "NVX Scan Freq")
         .options({{"30 Hz", 0}, {"80 Hz", 1}})
         .width(100.0f);
...
void AdminSettingsView::render(const AdminSettingsViewData& data, 
                                const AdminSettingsViewCallbacks& callbacks) {
    const ImGuiViewport* viewport = ImGui::GetMainViewport();
    ImGui::SetNextWindowPos(viewport->Pos);
    ImGui::SetNextWindowSize(viewport->Size);

    ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(0, 0));
    ImGui::Begin("##AdminSettingsScreen", nullptr,
...
void AdminSettingsView::render_tab_bar(const AdminSettingsViewData& data,
                                        const AdminSettingsViewCallbacks& callbacks) {
    tab_bar_.set_active_tab(data.active_tab);
    
    tab_bar_.set_on_tab_click([&callbacks](int index, const elda::ui::Tab&) {
...
void AdminSettingsView::render_configuration_tab() {
    ...
    float padding = (available_width > max_form_width) 
                    ? (available_width - max_form_width) / 2.0f 
                    : 0.0f;
    ...
    if (auto* f = form_.get_field("nvx_mode")) f->render(label_width);
    ImGui::Spacing();
    if (auto* f = form_.get_field("nvx_rate")) f->render(label_width);
    ImGui::Spacing();
    if (auto* f = form_.get_field("nvx_decimation")) f->render(label_width);
    ImGui::Spacing();
    if (auto* f = form_.get_field("nvx_gain")) f->render(label_width);
    ImGui::Spacing();
    if (auto* f = form_.get_field("nvx_power_save")) f->render(label_width);
    ImGui::Spacing();
    if (auto* f = form_.get_field("nvx_scan_freq")) f->render(label_width);
    ...
    if (auto* f = form_.get_field("output_file_type")) f->render(label_width);
    ImGui::Spacing();
    if (auto* f = form_.get_field("base_adc_sync")) f->render(label_width);
    ImGui::Spacing();
    if (auto* f = form_.get_field("sw_impedance_reduction")) f->render(label_width);
    ...
    if (auto* f = form_.get_field("channel_source")) f->render(label_width);
    ImGui::Spacing();
    if (auto* f = form_.get_field("channel_hpf")) f->render(label_width);
    ImGui::Spacing();
    if (auto* f = form_.get_field("channel_lpf")) f->render(label_width);
    ImGui::Spacing();
    if (auto* f = form_.get_field("channel_adf")) f->render(label_width);
