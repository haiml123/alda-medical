...
void UserSettingsView::setup_form() {
    ...
    form_.add_card_select("mode", "")
         .options({
             {"Normal", "Standard EEG", 0},
             {"MRI", "MR-compatible", 1},
             {"Custom", "User-defined", 2}
         })
         .card_size(140.0f, 80.0f)
         .spacing(16.0f);

    // Patient fields
    form_.add_text("technician", "Technician")
         .width(250.0f);

    form_.add_text("subject_code", "Subject Code")
         .width(250.0f)
         .required()
         .min_length(2, "At least 2 characters");

    form_.add_text("age", "Age")
         .width(80.0f)
         .required()
         .pattern("^[0-9]+$", "Must be a number");

    form_.add_multiline("notes", "Notes")
         .size(250.0f, 80.0f);
    ...
    form_.add_select("nvx_scan_freq", "Scan Frequency")
...
void UserSettingsView::render(const UserSettingsViewData& data,
                               const UserSettingsViewCallbacks& callbacks) {
    const ImGuiViewport* viewport = ImGui::GetMainViewport();
    ImGui::SetNextWindowPos(viewport->Pos);
    ImGui::SetNextWindowSize(viewport->Size);

    ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(0, 0));
    ImGui::Begin("##UserSettingsScreen", nullptr,
...
void UserSettingsView::render_content() {
    ...
    if (auto* f = form_.get_field("technician")) f->render(label_width);
    ImGui::Spacing();
    if (auto* f = form_.get_field("subject_code")) f->render(label_width);
    ImGui::Spacing();
    if (auto* f = form_.get_field("age")) f->render(label_width);
    ImGui::Spacing();
    if (auto* f = form_.get_field("notes")) f->render(label_width);
    ...
    if (is_custom) {
        ...
        if (auto* f = form_.get_field("nvx_mode")) f->render(hw_label_width);
        ImGui::Spacing();
        if (auto* f = form_.get_field("sampling_rate")) f->render(hw_label_width);
        ImGui::Spacing();
        if (auto* f = form_.get_field("nvx_gain")) f->render(hw_label_width);
        ImGui::Spacing();
        if (auto* f = form_.get_field("nvx_power_save")) f->render(hw_label_width);
        ImGui::Spacing();
        if (auto* f = form_.get_field("nvx_scan_freq")) f->render(hw_label_width);
        ImGui::Spacing();
        if (auto* f = form_.get_field("base_adc_sync")) f->render(hw_label_width);
        ImGui::Spacing();
        if (auto* f = form_.get_field("sw_impedance_reduction")) f->render(hw_label_width);
    }
