...
inline const char* acquisition_mode_to_string(AcquisitionMode mode) {
    switch (mode) {
        ...
        case AcquisitionMode::MRI:    return "MRI";
        case AcquisitionMode::CUSTOM: return "Custom";
...
public:
    ...
    // Mode
    AcquisitionMode get_mode() const { return mode_; }
    void set_mode(AcquisitionMode mode) { mode_ = mode; }

    // Data accessors
    PatientInfo& patient() { return patient_; }
    const PatientInfo& patient() const { return patient_; }

    MRISettings& mri_settings() { return mri_settings_; }
    const MRISettings& mri_settings() const { return mri_settings_; }

    CustomSettings& custom_settings() { return custom_settings_; }
    const CustomSettings& custom_settings() const { return custom_settings_; }
