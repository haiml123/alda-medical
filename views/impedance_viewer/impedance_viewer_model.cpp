...
void ImpedanceViewerModel::initialize_from_channels() {
    ...
    std::cout << "[ImpedanceViewerModel] Initialized with " << electrode_positions_.size()
              << " electrode positions\n";
}
...
const elda::models::Channel* ImpedanceViewerModel::get_channel_by_id(const std::string& id) const {
    auto it = std::find_if(
        available_channels_.begin(),
        available_channels_.end(),
        [&id](const elda::models::Channel& ch){ return ch.id == id; }
    );
...
void ImpedanceViewerModel::update_electrode_position(size_t index, float x, float y) {
    if (index >= electrode_positions_.size()) return;
    if (!is_position_valid(x, y)) return;

    electrode_positions_[index].x = x;
    electrode_positions_[index].y = y;

    std::cout << "[ImpedanceViewerModel] Updated electrode " << index
              << " to (" << x << ", " << y << ")\n";
...
void ImpedanceViewerModel::save_positions_to_state() {
    ...
    for (const auto& pos : electrode_positions_) {
        if (pos.channel_id.empty()) continue;
        ...
        std::cout << "[ImpedanceViewerModel] Saved impedance pos for channel "
                  << pos.channel_id << "\n";
    }
