...
void ImpedanceViewerView::render(const ImpedanceViewerViewData& data, const ImpedanceViewerViewCallbacks& callbacks)
{
    ...
    ImGui::Begin("ImpedanceViewer",
                 nullptr,
                 ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize |
                     ImGuiWindowFlags_NoBringToFrontOnFocus);

    // Header using reusable component
    ui::render_screen_header({.title = "Impedance Setup",
                              .show_back_button = true,
                              .on_back = callbacks.on_back,
                              .buttons = {{.label = "SETTINGS",
                                      ...
                                      const ImpedanceViewerViewCallbacks& callbacks)
{
    ...
    center_pos_ = ImVec2(canvas_pos_.x + canvas_size_.x * 0.5f, canvas_pos_.y + available_cap_height * 0.5f);
    ...
    ImGui::InvisibleButton(
        "impedance_canvas", canvas_size_, ImGuiButtonFlags_MouseButtonLeft | ImGuiButtonFlags_MouseButtonRight);
    const bool canvas_hovered = ImGui::IsItemHovered();

    draw_cap_outline(draw_list, center_pos_, pixel_cap_radius_);
    if (k_show_grid_default_)
        draw_cap_grid(draw_list, center_pos_, pixel_cap_radius_);

    render_electrodes(draw_list, *data.electrodes, *data.available_channels, data.selected_electrode_index, callbacks);
    ...
    if (canvas_hovered && ImGui::IsMouseClicked(ImGuiMouseButton_Left))
    {
        ...
        for (const auto& electrode : *data.electrodes)
        {
            ...
            if (point_in_circle(mp, p, k_electrode_radius_px_))
            {
                hit = true;
                break;
            }
    ...
    //
    {
        ...
        ImVec2 panel_pos(center_pos_.x - panel_width * 0.5f, center_pos_.y + pixel_cap_radius_ + panel_padding);
        ...
        elda::ui::draw_impedance_range_dual(
            "imp-range-dual", low_ohm, high_ohm, ranges, bar_cfg, cursor_cfg, &low_ohm, &high_ohm);

        ImGui::EndChild();
    }
}

void ImpedanceViewerView::render_electrodes(ImDrawList* draw_list,
                                            ...
                                            const ImpedanceViewerViewCallbacks& callbacks)
{
    for (size_t i = 0; i < electrodes.size(); ++i)
    {
        ...
        {
            auto it = std::find_if(available_channels.begin(),
                                   available_channels.end(),
                                   [&](const elda::models::Channel& c)
                                   {
                                       return c.id == electrode.channel_id;
                                   });
        ...
        render_single_electrode(draw_list, i, electrode, channel, is_selected, callbacks);
    }
}

void ImpedanceViewerView::render_single_electrode(ImDrawList* draw_list,
                                                  size_t index,
                                                  ...
                                                  const ImpedanceViewerViewCallbacks& callbacks)
{
    ...
    draw_list->AddText(ImVec2(pos.x - label_size.x * 0.5f, pos.y - label_size.y * 0.5f), IM_COL32(0, 0, 0, 255), label);
}
