...
void MonitoringPresenter::on_enter() {
    ...
    model_.add_state_observer([this](StateField field) {
...
void MonitoringPresenter::render() {
    ...
    if (channels_presenter_->is_open()) {
        ...
        ImVec2 modal_pos = use_custom_modal_position_
            ? channel_modal_position_
            : calculate_default_modal_position(modal_size);
...
void MonitoringPresenter::setup_callbacks() {
    callbacks_.on_toggle_monitoring   = [this]() { model_.toggle_monitoring();   };
    callbacks_.on_toggle_recording    = [this]() {
        model_.toggle_recording();
    };
    callbacks_.on_stop_recording      = [this]() { model_.stop_recording();      };
    callbacks_.on_increase_window     = [this]() { model_.increase_window();     };
    callbacks_.on_decrease_window     = [this]() { model_.decrease_window();     };
    callbacks_.on_increase_amplitude  = [this]() { model_.increase_amplitude();  };
    callbacks_.on_decrease_amplitude  = [this]() { model_.decrease_amplitude();  };
