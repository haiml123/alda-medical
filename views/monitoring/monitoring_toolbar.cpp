...
// ======================= monitor button =======================
static void
render_monitor_button(const MonitoringViewData& data, const MonitoringViewCallbacks& callbacks, float toolbar_h)
{
    ...
    {
        if (callbacks.on_toggle_monitoring)
            callbacks.on_toggle_monitoring();
    }
...
static bool icon_button(const char* id,
                        ImVec2 size,
                        ImU32 halo_col,
                        float halo_rounding,
                        float icon_scale,
...
// ======================= recording cluster =======================
static void
render_recording_controls(const MonitoringViewData& data, const MonitoringViewCallbacks& callbacks, float toolbar_h)
{
    ...
    if (!can_record)
        ImGui::BeginDisabled();
    bool toggled = icon_button("##rec_toggle",
                               icon_size,
                               halo_col,
                               halo_round,
                               icon_scale,
                               [&](ImDrawList* dl, ImVec2 min, ImVec2 max, float scale)
                               {
                                   ...
                                   if (data.recording_state == RecordingState::Recording)
                                       draw_pause_icon(dl, center, h, col_pause);
                                   else
                                       draw_play_icon(dl, center, h, col_play);
                               });
    if (!can_record)
        ImGui::EndDisabled();
    if (toggled && callbacks.on_toggle_recording)
        callbacks.on_toggle_recording();
    ...
    if (!can_stop)
        ImGui::BeginDisabled();
    bool stop_pressed = icon_button("##rec_stop",
                                    icon_size,
                                    halo_col,
                                    halo_round,
                                    icon_scale,
    ...
    if (!can_stop)
        ImGui::EndDisabled();
    if (stop_pressed && callbacks.on_stop_recording)
...
// -----------------------------------------------------------------------------
static void
render_window_controls(const MonitoringViewData& data, const MonitoringViewCallbacks& callbacks, float /*toolbar_h*/)
{
    ImGui::SameLine();
    ImGui::TextDisabled("|");
    ...
    {
        if (callbacks.on_decrease_window)
            callbacks.on_decrease_window();
    }
    ...
    if (square_button("+##win"))
    {
        if (callbacks.on_increase_window)
            callbacks.on_increase_window();
    }
}
...
// -----------------------------------------------------------------------------
static void
render_amplitude_controls(const MonitoringViewData& data, const MonitoringViewCallbacks& callbacks, float /*toolbar_h*/)
{
    ImGui::SameLine();
    ...
    {
        if (callbacks.on_decrease_amplitude)
            callbacks.on_decrease_amplitude();
    }

    ImGui::SameLine();
    ImGui::AlignTextToFramePadding();  // << aligns label with buttons
    if (data.amplitude_micro_volts == 1000)
        ImGui::Text("1 mV");
    else
        ImGui::Text("%d µV", data.amplitude_micro_volts);

    ImGui::SameLine();
    if (square_button("+##amp"))
    {
        if (callbacks.on_increase_amplitude)
            callbacks.on_increase_amplitude();
    }
}
...
static void render_impedance_button(const MonitoringViewCallbacks& callbacks)
{
    ...
    {
        if (callbacks.on_open_impedance_viewer)
            callbacks.on_open_impedance_viewer();
    }
...
static void render_status_info(const MonitoringViewData& data, float /*toolbar_h*/)
{
    ...
    else if (data.recording_state == RecordingState::Paused)
