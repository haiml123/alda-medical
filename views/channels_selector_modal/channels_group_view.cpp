...
ChannelsGroupView::ChannelsGroupView() : is_visible_(false), position_(0, 0), size_(300, 550)
{
...
void ChannelsGroupView::render(const std::string& group_name,
...
void ChannelsGroupView::render_channels_list(const std::vector<models::Channel>& channels,
                                             int selected_count,
                                             int total_count)
{
    // Section title with count
    char section_title[64];
    std::snprintf(section_title, sizeof(section_title), "Select Channels (%d / %d)", selected_count, total_count);
