...
std::string MultiSelect::FormatButtonLabel(const char* label, int count) const
{
    ...
    if (m_buttonLabelFormat.find("%d") != std::string::npos && m_buttonLabelFormat.find("%s") != std::string::npos)
    {
...
bool MultiSelect::Draw(const char* label)
{
    ...
    if (m_isOpen)
    {
                             ...
                             ImGuiWindowFlags_NoSavedSettings))
        {
            ...
            ImVec2 windowMax = ImVec2(windowMin.x + ImGui::GetWindowSize().x, windowMin.y + ImGui::GetWindowSize().y);
